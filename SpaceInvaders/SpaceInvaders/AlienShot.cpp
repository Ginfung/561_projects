...
GLMmodel* AlienShot::loadModel(char *objFileName){
...
AlienShot::AlienShot(float offsetX, float offsetZ, GLMmodel* o){
	//loadModel(".//inputs//bulletfrominvader.obj");
	this->object = o;
...
AlienShot::~AlienShot(void){
	delete object;
}

void AlienShot::displayAlienShot(){
	...
	while (group){
		...
		for (GLuint i = 0; i < group->numtriangles; i++){
			...
			glVertex3f(v1[0] + offsetX, v1[1] , v1[2] + offsetZ);
			glVertex3f(v2[0] + offsetX, v2[1] , v2[2] + offsetZ);
			glVertex3f(v3[0] + offsetX, v3[1] , v3[2] + offsetZ);
